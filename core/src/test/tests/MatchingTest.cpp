...
static void testRegexpTokens() {
	// Regexp tokens
	assertMatches( "Мама", 0, 1, "Act = \"мама\"" );
	assertMatches( "Мама", 0, 1, "Act = \"ма.*а\"" );
	assertMatches( "Маша", 0, 1, "Act = \"ма.*а\"" );
	assertMatches( "Мамаша", 0, 1, "Act = \"ма.*а\"" );
	assertNoMatches( "Мошка", "Act = \"ма.*а\"" );
}
...
static void testRestrictionsOnBase() {
	assertMatches( "Мама мыла раму", 0, 2, "Act = N V <lemma=МЫТЬ>" );
	assertMatches( "Мама мыла раму", 0, 2, "Act = N V <МЫТЬ>" );
	assertMatches( "Мама мыла раму", 1, 2, "Act = V <!=БЕГАТЬ>");
	assertMatches( "Мама мыла раму", 1, 2, "Act = V <\".*Ы.*\">");
	assertNoMatches( "Мама мыла раму", "Act = V <\".*з.*\">");
	assertNoMatches( "Мама мыла раму", "Act = V <!=МЫТЬ>" );
	assertNoMatches( "Мама мыла раму", "Act = N V <lemma=БЕГАТЬ>" );
	assertNoMatches( "Мама мыла раму", "Act = N V<БЕГАТЬ>" );
}

static void testRestrictionsOnStem() {
	assertMatches( "Мама мыла раму", 0, 2, "Act = N V <stem=М>" );
	assertMatches( "Мама мыла раму", 0, 2, "Act = N V <stem=\".\">" );
	assertNoMatches( "Мама мыла раму", "Act = N V <stem=БЕГ>" );
	assertMatches( "Мама мыла раму", 1, 2, "Act = V <stem!=БЕГ>" );
	assertNoMatches( "Мама мыла раму", "Act = N V <stem=БЕГ>" );
}

static void testTerm() {
	// Term
	assertMatches( "Процессор базы данных", 0, 3, "Act = \"процессор\" \"базы\" \"данных\"" );
	assertMatches( "Процессор базы данных", 0, 3, "Act = \"процессор базы данных\"" );
	assertMatches( "Процессор базы данных", 0, 3, "Act = N1<процессор> { \"базы\" \"данных\" | \"ввода-вывода\" }<1,1>" );
}
...
static void testReusing1() {
	...
	assertMatchesNS( ns, "Белая кошка шла по дорожке", 0, 1, "AA (A, Pa) = A | Pa" );
...
static void testReusing2() {
	...
	assertNoMatchesNS( ns, "Мама мыла раму", "AA(A, Pa) = A | Pa" );
...
static void testStrictAgreement() {
	assertMatches( "Мой Сон", 0, 2, "Pattern = W1 W2 <<W1=W2>>" );
	assertNoMatches( "Мой Сон", "Pattern = W1 W2 <<W1==W2>>" );
	assertMatches( "Сон Сон", 0, 2, "Pattern = W1 W2 <<W1==W2>>" );
}
...
static void testRestrictedMatches() {
	...
	assertMatches( "Мама мыла раму", 0, 2, "Act = N1 V1" );
	assertMatches( "Черный кот шел сам по себе", 1, 3, "Act = N1 V1" );

	assertRestrictedMatches( "Мама мыла раму", 0, 2, "Act = N1 V1", r );
	assertNoRestrictedMatches( "Черный кот шел сам по себе", "Act = N1 V1", r );
}
